...
pub fn MouseMoveComponent(props: &MouseMoveProps) -> Html {
	...
	let on_bottom_left_resizer_move = {
		...
		move |event: MouseEvent| {
			...
			let mut new_width = *resize_start_width - dx;
			let mut new_height = (new_width as f64 / aspect_ratio) as i32;
			let mut new_x = *drag_start_left + dx;
			let mut new_y = *drag_start_top + dy;
